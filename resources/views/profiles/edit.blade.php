        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('profiles.update', $profile) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="category" name="category" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category }}" {{ old('category', $profile->category) == $category ? 'selected' : '' }}>{{ $category }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="city" class="block mt-1 w-full" type="text" name="city" :value="old('city', $profile->city)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $profile->phone)" required />
                            ...
                            <div>
                                ...
                                @if($profile->photo_path)
                                    <div class="mt-2 mb-3">
                                        <img src="{{ Storage::url($profile->photo_path) }}" alt="{{ $profile->user->name }}" class="h-24 w-24 object-cover rounded">
                                    </div>
                                @endif
                                    ...
                                    <div class="flex items-start">
                                        <div class="flex items-center h-5">
                                            <input id="service_{{ $service->id }}" name="services[]" value="{{ $service->id }}" type="checkbox" class="w-4 h-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500"
                                                {{ (in_array($service->id, old('services', [])) || $profile->services->contains($service->id)) ? 'checked' : '' }}>
                                        </div>
                        ...
                        <div class="flex items-center justify-between mt-4">
                            <form method="POST" action="{{ route('profiles.destroy', $profile) }}" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150" onclick="return confirm('Вы уверены, что хотите удалить свой профиль?')">
                                    {{ __('Удалить профиль') }}
                                </button>
                            </form>
